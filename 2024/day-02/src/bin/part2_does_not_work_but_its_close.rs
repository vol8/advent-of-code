fn check_if_dec_inc(nums_in_row: &Vec<u32>) -> bool {
    ...
    for n in nums_in_row.iter() {
        ...
        if is_inc && nums_in_row[idx] > nums_in_row[idx+1] {
            if nums_in_row.len() == idx+2 {
                continue;
            }

            if nums_in_row[idx] > nums_in_row[idx+2] || tolerated_once {
                return false;
            } else {
                tolerated_once = true;
            }
...
fn check_inc_dec_val(nums_in_row: &Vec<u32>) -> bool {
    ...
    for n in nums_in_row.iter() {
        ...
        if nums_in_row[idx] > nums_in_row[idx+1] && nums_in_row[idx] - nums_in_row[idx+1] >= 4 {
            ...
            if nums_in_row[idx] > nums_in_row[idx+2] && nums_in_row[idx] - nums_in_row[idx+2] >= 4 || tolerated_once {
                return false;
            } else {
                tolerated_once = true;
            }
            ...
            if nums_in_row[idx] < nums_in_row[idx+2] && nums_in_row[idx+2] - nums_in_row[idx] >= 4 || tolerated_once {
                return false;
            } else {
                tolerated_once = true;
            }
